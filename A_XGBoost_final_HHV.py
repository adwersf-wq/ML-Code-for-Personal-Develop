...
# # 参数网格（根据XGBoost调整）
# param_grid = {
#     'regressor__xgb__n_estimators': [100, 150, 200, 250, 300, 400, 500],
#     'regressor__xgb__max_depth': [1, 3, 6, 9, 12],
#     'regressor__xgb__learning_rate': [0.01, 0.05, 0.1, 0.2],
#     'regressor__xgb__subsample': [0.4, 0.6, 0.8, 1.0],
#     'regressor__xgb__colsample_bytree': [0.4,0.6, 0.8, 1.0]
# }
#
# # 网格搜索
...
if train_mse < 1e-10 or train_r2 > 0.9999:
    ...
    print(f"   1. 降低 learning_rate 或增加 n_estimators")
    print(f"   2. 增加正则化参数 (如 reg_alpha, reg_lambda)")
    print(f"   3. 降低 max_depth 或增大 subsample/colsample_bytree")
elif gap_r2 > 0.15:
    ...
    print(f"   - 降低 learning_rate 并增加 n_estimators")
    print(f"   - 降低 max_depth")
    print(f"   - 增大 subsample 或 colsample_bytree")
elif gap_r2 > 0.05:
...
plt.title('XGBoost模型验证图（HHV）', fontsize=22)
...
plt.tight_layout()
plt.savefig('XGBoost_model_validation.png', dpi=300, bbox_inches='tight')
plt.show()
print("模型验证图已保存为 'XGBoost_model_validation.png'")
...
# 从最佳模型中提取XGB模型和标准化器
xgb_model = best_model.regressor_.named_steps['xgb']
...
plt.tight_layout()
plt.savefig('XGB_feature_importance_bar.png', dpi=300, bbox_inches='tight')
plt.show()
print("特征重要性柱状图已保存为 'XGB_feature_importance_bar.png'")
...
ax.set_title('XGBoost模型SHAP特征值分布（HHV）', fontsize=14, fontweight='bold')

plt.tight_layout()
plt.savefig('XGB_shap_beeswarm.png', dpi=300, bbox_inches='tight')
plt.show()
print("SHAP蜂群图已保存为 'XGB_shap_beeswarm.png'")
...
# file_pairs = [
#     ("new_data_corncob.xlsx", "prediction_results_XGB_corncob_HHV.xlsx"),
#     ("new_data_wheat_shell.xlsx", "prediction_results_XGB_wheat_shell_HHV.xlsx"),
#     ("new_data_walnut_shell.xlsx", "prediction_results_XGB_walnut_shell_HHV.xlsx")
# ]
#
